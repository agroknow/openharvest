$(document).ready(function () {
        ...
        $.getJSON( "http://maps.local/map-data-js", function( result ) {
             mapData = result;
                ...
                $.getJSON( "http://maps.local/networks-data", function( orgdata ) {
                    relations = orgdata;
        ...
        var markerLayerOptions = {
            ...
            setIcon: function (record, options) {
                var html = '<div><i class="icon-building"></i><!--<span class="code">' + record.title + '</span>--></div>';
                var $html = $(html);
                var $i = $html.find('i');
        ...
        layerControl.addOverlay(markers, 'Collections');
